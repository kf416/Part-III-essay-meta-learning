...
class ClassificationModel:
    ...
    likelihood_std = None
    num_classes = None  # added parameter
    ...
    def predict(self, x, sample_shape=()):
        x = self._handle_input_data(x, convert_to_tensor=True)
        logits = self.nn.predict(x, sample_shape=sample_shape)
        probs = tf.nn.softmax(logits, axis=-1)
        pred_dist = tfd.Categorical(probs=probs)
    ...
    def plot_predictions(self, x_plot, show=False, plot_train_data=True, ax=None):
        ...
        assert self.input_dim == 1 and self.output_dim == self.num_classes
    ...
    def _process_train_data(self, x_train, y_train, normalization_stats=None):
        self.x_train, self.y_train = self
